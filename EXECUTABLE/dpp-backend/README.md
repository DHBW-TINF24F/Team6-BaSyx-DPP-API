...
| ✅ | FR-BE-08 | Registrierung im AAS Registry          | DPP registrieren                                 | DPP ist im Registry auffindbar                      | `curl -X POST http://api.com/registry -H "Content-Type: application/json" -d '{ "shell": { "id": "urn:uuid:7e51f712-429a-419a-9e8c-8f43c393850b", "dpps": [ { "productId": "PID", "version": "1.0.0"} }'` | `POST /registry` |
| ✅ | FR-BE-09 | Submodel-Daten abrufen                 | Submodel per elementId abrufen                   | Daten des Elements liegen vor                       | `curl http://api.com/dpps/{dppId}/submodels/{elementId}` | `GET /dpps/{dppId}/submodels/{elementId}` |
| ✅| FR-BE-10 | Submodel-Daten updaten                 | Submodel aktualisieren                           | Daten wurden angepasst                              | `curl -X PATCH http://api.com/dpps/{dppId}/submodels/{elementId} -H "Content-Type: application/json" -d '{...}'` | `PATCH /dpps/{dppId}/submodels/{elementId}` |
| ✅ | FR-BE-11 | Element per elementPath abrufen        | Einzelnes Element abrufen                        | Element liegt korrekt vor                           | `curl http://api.com/dpps/{dppId}/element/{elementPath}` | `GET /dpp/{dppId}/elements/{elementPath}` |
